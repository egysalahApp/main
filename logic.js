// ================= الأيقونات =================
        const ICONS = {
            youtube: `<svg class="w-7 h-7" fill="#FF0000" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.582,6.186c-0.23-0.86-0.908-1.538-1.768-1.768C18.254,4,12,4,12,4S5.746,4,4.186,4.418 c-0.86,0.23-1.538,0.908-1.768,1.768C2,7.746,2,12,2,12s0,4.254,0.418,5.814c0.23,0.86,0.908,1.538,1.768,1.768 C5.746,20,12,20,12,20s6.254,0,7.814-0.418c0.86-0.23,1.538-0.908,1.768-1.768C22,16.254,22,12,22,12S22,7.746,21.582,6.186z M10,15.464V8.536L16,12L10,15.464z"/></svg>`,
...
// 🛑 [AI GUARD WARNING]: توقف هنا! ممنوع منعاً باتاً تعديل أي سطر برمجي أو دالة JavaScript أو CSS أسفل هذا السطر نهائياً.
        ...
        const App = {
            ...
            showSectionResult(sectionId, sectionDef, progress) {
                ...
                if (resultArea) {
                    resultArea.innerHTML = `<div class="mt-12 p-8 md:p-10 bg-white border-2 border-${sectionDef.theme}-200 rounded-2xl shadow-sm text-center max-w-2xl mx-auto w-full smooth-expand"><h3 class="text-2xl md:text-3xl font-semibold text-slate-700 mb-6">نتيجتك في قسم (${sectionDef.title})</h3><div class="text-4xl md:text-5xl font-bold text-${sectionDef.theme}-600 flex items-center justify-center gap-3"><span>${toArabicNum(progress.score)}</span><span class="text-2xl md:text-3xl text-slate-400 font-medium mt-1">من</span><span>${toArabicNum(progress.total)}</span></div></div>`;
                }
            ...
            renderStandardSection(sectionData) {
                ...
                if (isComplete) { html += `<div class="mt-12 p-8 md:p-10 bg-white border-2 border-${sectionData.theme}-200 rounded-2xl shadow-sm text-center max-w-2xl mx-auto w-full"><h3 class="text-2xl md:text-3xl font-semibold text-slate-700 mb-6">نتيجتك في قسم (${sectionData.title})</h3><div class="text-4xl md:text-5xl font-bold text-${sectionData.theme}-600 flex items-center justify-center gap-3"><span>${toArabicNum(progress.score)}</span><span class="text-2xl md:text-3xl text-slate-400 font-medium mt-1">من</span><span>${toArabicNum(progress.total)}</span></div></div>`; }
                html += '</div></div>';
                ...
                <div id="${sectionId}-q-${idx}" class="bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-slate-200 mb-8 relative transition-colors duration-300 flex flex-col justify-start ${ringClass}">
                    <div class="bg-slate-50 p-4 md:p-6 rounded-xl border border-slate-100 mb-6 w-full mx-auto">
                        ...
                        <div id="hint-area-${sectionId}-${idx}" class="${showHint ? `block w-full ${hintFadeClass}` : 'hidden'} mt-2">
                            <div class="bg-amber-50 text-amber-900 p-4 rounded-xl text-xl font-medium border border-amber-200 flex gap-2 items-start text-right">
                                <span class="text-amber-500 font-semibold mt-1 shrink-0">💡</span><span class="leading-relaxed">${q.hint}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="feedback-area-${sectionId}-${idx}" class="${answered ? `block w-full ${answerFadeClass}` : 'hidden'}">
                        <div class="mt-6 p-5 rounded-xl border text-right ${isSelectedCorrect ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}">
                            <div class="flex items-center justify-start gap-2 mb-3 font-bold text-xl md:text-2xl ${isSelectedCorrect ? 'text-emerald-700' : 'text-rose-700'}">
                                ${isSelectedCorrect ? `<span class="text-emerald-700">${ICONS.checkCircleLarge}</span>` : `<span class="text-rose-700">${ICONS.xCircleLarge}</span>`}
                                <span>${isSelectedCorrect ? 'إجابة صحيحة' : 'إجابة خاطئة'}</span>
                            </div>
                            <div class="text-xl md:text-2xl font-semibold leading-relaxed ${isSelectedCorrect ? 'text-emerald-800' : 'text-rose-800'} pr-1">${q.explanation}</div>
                        </div>
                    </div>
                </div>`;
            },
            ...
            renderSortInner(sectionData) {
                ...
                html += `<div class="relative w-full min-h-[12rem] bg-slate-50 rounded-3xl border-2 border-dashed border-slate-300 flex flex-col items-center justify-center p-4 md:p-6 transition-all duration-500">${!isComplete ? `<div class="w-full max-w-2xl bg-white p-5 md:p-8 rounded-2xl shadow-lg border-2 border-slate-200 transition-all transform leading-snug flex flex-col justify-start z-10 ${animatingOut ? 'fly-out' : 'fade-in'} ${status === 'incorrect' ? 'shake' : ''}"><div class="flex items-center justify-between mb-4"><span class="text-${theme}-600 font-bold text-lg md:text-xl">البطاقة ${toArabicNum(currentIndex + 1)}</span><button onclick="App.toggleHint('${sectionId}', ${currentIndex})" class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-lg md:text-xl font-bold transition-all active:scale-95 text-amber-500 md:hover:bg-amber-50">💡 تلميح</button></div><h3 class="text-3xl md:text-4xl font-bold leading-snug text-slate-800 text-center">«${currentQState.originalQuestion.text}»</h3><div id="hint-area-${sectionId}-${currentIndex}" class="${showHint ? `block w-full ${hintFadeClass}` : 'hidden'} mt-4"><div class="bg-amber-50 text-amber-900 p-4 rounded-xl text-xl font-medium border border-amber-200 flex gap-2 items-start text-right"><span class="text-amber-500 font-semibold mt-1 shrink-0">💡</span><span class="leading-relaxed">${currentQState.originalQuestion.hint}</span></div></div><div class="${status === 'incorrect' ? 'block w-full smooth-expand' : 'hidden'} mt-5"><div class="p-4 bg-rose-50 border border-rose-200 rounded-xl text-center text-rose-700 text-lg md:text-xl font-bold shadow-sm">⚠️ إجابة خاطئة! ${currentQState.originalQuestion.explanation}</div></div></div>` : `<div class="text-3xl md:text-4xl font-bold text-emerald-500 flex flex-col items-center gap-4 fade-in py-6"><span class="text-6xl">🎉</span><span>اكتملت فرز البطاقات!</span></div>`}</div>${!isComplete ? `<div class="flex flex-row w-full max-w-2xl mx-auto gap-2 md:gap-4 mt-2 relative z-0 flex-wrap justify-center">${boxesHTML}</div>` : ''}</div>`;
                if (isComplete) { const progress = this.state.progress[sectionId]; html += `<div class="mt-12 p-8 md:p-10 bg-white border-2 border-${theme}-200 rounded-2xl shadow-sm text-center smooth-expand max-w-2xl mx-auto w-full"><h3 class="text-2xl md:text-3xl font-semibold text-slate-700 mb-6">نتيجتك في قسم (${sectionData.title})</h3><div class="text-4xl md:text-5xl font-bold text-${theme}-600 flex items-center justify-center gap-3"><span>${toArabicNum(progress.score)}</span><span class="text-2xl md:text-3xl text-slate-400 font-medium mt-1">من</span><span>${toArabicNum(progress.total)}</span></div></div>`; }
            ...
            renderSpottingCard(qState, idx, theme, sectionId) {
                ...
                <div id="${sectionId}-q-${idx}" class="bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-slate-200 mb-8 relative transition-colors duration-300 flex flex-col justify-start ${ringClass}">
                    ...
                    <div id="feedback-area-${sectionId}-${idx}" class="${answered ? `block w-full ${answerFadeClass}` : 'hidden'}">
                        <div class="mt-6 p-5 rounded-xl border text-right bg-emerald-50 border-emerald-200">
                            <div class="flex items-center justify-start gap-2 mb-3 font-bold text-xl md:text-2xl text-emerald-700">
                                <span class="text-emerald-700">${ICONS.checkCircleLarge}</span><span>أحسنت الاختيار</span>
                            </div>
                            <div class="text-xl md:text-2xl font-semibold leading-relaxed text-emerald-800 pr-1">${q.explanation}</div>
                        </div>
                    </div>
                </div>`;
            },
            ...
            renderHotspotCard(qState, idx, theme, sectionId) {
                ...
                <div id="${sectionId}-q-${idx}" class="bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-slate-200 mb-8 relative transition-colors duration-300 flex flex-col justify-start ${ringClass}">
                    ...
                    <div id="feedback-area-${sectionId}-${idx}" class="${answered ? `block w-full ${answerFadeClass}` : 'hidden'}">
                        <div class="mt-6 p-5 rounded-xl border text-right bg-emerald-50 border-emerald-200">
                            <div class="flex items-center justify-start gap-2 mb-3 font-bold text-xl md:text-2xl text-emerald-700">
                                <span class="text-emerald-700">${ICONS.checkCircleLarge}</span><span>أحسنت الاستخراج!</span>
                            </div>
                            <div class="text-xl md:text-2xl font-semibold leading-relaxed text-emerald-800 pr-1">${q.explanation}</div>
                        </div>
                    </div>
            ...
            renderMatching(sectionData) {
                ...
                if (isComplete) {
                    html += `<div class="mt-12 p-8 md:p-10 bg-white border-2 border-${sectionData.theme}-200 rounded-2xl shadow-sm text-center max-w-2xl mx-auto w-full"><h3 class="text-2xl md:text-3xl font-semibold text-slate-700 mb-6">نتيجتك في قسم (${sectionData.title})</h3><div class="text-4xl md:text-5xl font-bold text-${sectionData.theme}-600 flex items-center justify-center gap-3"><span>${toArabicNum(progress.score)}</span><span class="text-2xl md:text-3xl text-slate-400 font-medium mt-1">من</span><span>${toArabicNum(progress.total)}</span></div></div>`;
                }
                html += '</div>';
            ...
            renderFlashcardsInner(sectionData) {
                ...
                html += `</div></div>`;
                if (isComplete) { const progress = this.state.progress[sectionId]; html += `<div class="mt-8 p-8 md:p-10 bg-white border-2 border-${theme}-200 rounded-2xl shadow-sm text-center smooth-expand max-w-2xl mx-auto w-full"><h3 class="text-2xl md:text-3xl font-semibold text-slate-700 mb-6">نتيجتك في قسم (${sectionData.title})</h3><div class="text-4xl md:text-5xl font-bold text-${theme}-600 flex items-center justify-center gap-3"><span>${toArabicNum(progress.score)}</span><span class="text-2xl md:text-3xl text-slate-400 font-medium mt-1">من</span><span>${toArabicNum(progress.total)}</span></div></div>`; }
                html += `<div id="section-footer-area-${sectionId}" class="w-full">` + this.renderSectionFooterControls(sectionId, isComplete) + `</div>`;
                return html;
            },
                ...
                if (currentQuestion) {
                    ...
                    <div id="classify-card" class="w-full max-w-2xl bg-white p-5 md:p-8 rounded-2xl shadow-xl border-2 border-slate-200 leading-snug flex flex-col justify-start z-10 ${animatingOut ? 'fly-out' : 'fade-in'} ${shakeClass}">
                        ...
                        <div id="hint-area-${sectionId}-${currentIndex}" class="${showHint ? `block w-full mb-4 ${hintFadeClass}` : 'hidden'}">
                            <div class="bg-amber-50 text-amber-900 p-4 rounded-xl text-xl font-medium border border-amber-200 flex gap-2 items-start text-right">
                                <span class="text-amber-500 font-semibold mt-1 shrink-0">💡</span>
                                <span class="leading-relaxed">${currentQuestion.hint}</span>
                            </div>
                        </div>
                        <div class="${status === 'incorrect' ? 'block w-full smooth-expand' : 'hidden'} mb-4">
                            <div class="p-4 bg-rose-50 border border-rose-200 rounded-xl text-center text-rose-700 text-lg md:text-xl font-bold shadow-sm">⚠️ ${currentQuestion.explanation}</div>
                        </div>
                        ${actionButtonsHTML}
                    </div>`;
                } else {
            ...
            updateErrorCardPartial(sectionId, qIndex) {
                ...
                if (feedbackArea) {
                    if (isStep3) {
                        ...
                        feedbackArea.innerHTML = `<div class="mt-6 p-5 rounded-xl border text-right ${isSelectedCorrect ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}"><div class="flex items-center justify-start gap-2 mb-3 font-bold text-xl md:text-2xl ${isSelectedCorrect ? 'text-emerald-700' : 'text-rose-700'}">${isSelectedCorrect ? `<span class="text-emerald-700">${ICONS.checkCircleLarge}</span>` : `<span class="text-rose-700">${ICONS.xCircleLarge}</span>`}<span>${isSelectedCorrect ? 'إجابة صحيحة' : 'إجابة خاطئة'}</span></div><div class="text-xl md:text-2xl font-semibold leading-relaxed ${isSelectedCorrect ? 'text-emerald-800' : 'text-rose-800'} pr-1">${q.explanation}</div></div>`;
                    } else {
                ...
                if (mainCard) {
                    if (isStep3) {
                        ...
                        mainCard.classList.remove('ring-2', 'ring-emerald-400', 'ring-rose-400');
                    }
                }
            },
            ...
            updateSpottingCardPartial(sectionId, qIndex, theme) {
                ...
                if (feedbackArea) {
                    if (answered) {
                        feedbackArea.className = `block w-full ${answerAnimate ? 'smooth-expand' : ''}`;
                        feedbackArea.innerHTML = `<div class="mt-6 p-5 rounded-xl border text-right bg-emerald-50 border-emerald-200"><div class="flex items-center justify-start gap-2 mb-3 font-bold text-xl md:text-2xl text-emerald-700"><span class="text-emerald-700">${ICONS.checkCircleLarge}</span><span>أحسنت الاختيار</span></div><div class="text-xl md:text-2xl font-semibold leading-relaxed text-emerald-800 pr-1">${q.explanation}</div></div>`;
                    } else {
                        feedbackArea.className = 'hidden';
                    }
                }
            ...
            updateHotspotCardPartial(sectionId, qIndex, theme) {
                ...
                if (feedbackArea) {
                    if (answered) {
                        feedbackArea.className = `block w-full ${answerAnimate ? 'smooth-expand' : ''}`;
                        feedbackArea.innerHTML = `<div class="mt-6 p-5 rounded-xl border text-right bg-emerald-50 border-emerald-200"><div class="flex items-center justify-start gap-2 mb-3 font-bold text-xl md:text-2xl text-emerald-700"><span class="text-emerald-700">${ICONS.checkCircleLarge}</span><span>أحسنت الاستخراج!</span></div><div class="text-xl md:text-2xl font-semibold leading-relaxed text-emerald-800 pr-1">${q.explanation}</div></div>`;
                    } else {
                        feedbackArea.className = 'hidden';
                    }
                }
            ...
            handleErrorWordClick(sectionId, qIndex, wIndex) {
                ...
                const isCorrectError = wIndex === qState.originalQuestion.errorWordIndex;
                if (isCorrectError) {
            ...
            scrollToContent() { requestAnimationFrame(() => { setTimeout(() => { const target = document.getElementById('scroll-target'); if (target) { const stickyTabs = document.getElementById('sticky-tabs-container'); const headerOffset = stickyTabs ? stickyTabs.offsetHeight : 80; const elementPosition = target.getBoundingClientRect().top; const offsetPosition = elementPosition + window.pageYOffset - headerOffset; window.scrollTo({ top: offsetPosition, behavior: 'smooth' }); } }, 30); }); },
            scrollTabIntoView(tabId) { requestAnimationFrame(() => { setTimeout(() => { const tabBtn = document.getElementById(`tab-${tabId}`); const container = document.getElementById('tabs-container'); if (tabBtn && container) { const containerRect = container.getBoundingClientRect(); const btnRect = tabBtn.getBoundingClientRect(); if (tabId === APP_DATA.sections[0].id) { container.scrollBy({ left: (btnRect.right - containerRect.right) + 16, behavior: 'smooth' }); } else { container.scrollBy({ left: (btnRect.left + btnRect.width / 2) - (containerRect.left + containerRect.width / 2), behavior: 'smooth' }); } } }, 30); }); },
            ...
            handleResetSection(sectionId) {
                ...
                else if (secDef.type === 'error_correction') { this.state.sectionData[sectionId] = secDef.questions.map(q => ({ originalQuestion: q, options: shuffleArray(q.options.map((opt, oIdx) => ({ text: opt, isCorrect: oIdx === q.correctOptionIndex }))), step: 1, selectedWordIndex: null, selectedOption: null, showHint: false, hintAnimate: false, optionsAnimate: false, answerAnimate: false, shakeWordIndex: null })); }
            ...
            checkFinalEvaluation(forceShow = false) {
                ...
                if (canShow || forceShow) {
                    ...
                    if (percentage < 60) { message = "تحتاج إلى مزيد من التدريب، لا تيأس وحاول مرة أخرى!"; color = "text-rose-600"; } else if (percentage < 85) { message = "أداء رائع! مستواك جيد جدًا."; color = "text-violet-600"; } else { message = "أداء استثنائي! أنت بطل حقيقي وخبير في هذا الدرس."; color = "text-emerald-600"; }
